...
public class TwoDarrayMethods
{
    ...
    public static void main(String[] args)
    {
        ...
        System.out.println("  Row with greatest sum: " + rowWithGreatestSum(matrix)); //test code
        System.out.println("  Column with lowest sum: " + columnWithLowestSum(matrix));
        System.out.println("  Largest average for row/col 2: " + maxAverageRowOrCol(matrix, 2));
        System.exit(0);
    }
    ...
    public static int rowWithGreatestSum(double[][] matrix) {
        int maxRowIndex = 0; //row index varaible
        double maxSum = sumRow(matrix, 0); //call sumRow for the current max

        for (int row = 1; row < NUM_ROWS; row++) { //traverse rows
            double currentSum = sumRow(matrix, row); //get current row sum
            if (currentSum > maxSum) { //if larger then set max to current and row idnex to row
                maxSum = currentSum;
                maxRowIndex = row;
            }
        }
    ...
    public static int columnWithLowestSum(double[][] matrix) { //exact same process as above but with columnrs
        int minColIndex = 0;
        double minSum = sumCol(matrix, 0);

        for (int col = 1; col < NUM_COLS; col++) {
            double currentSum = sumCol(matrix, col);
    ...
    public static double maxAverageRowOrCol(double[][] matrix, int n) {
        if (n < 0 || n >= NUM_ROWS || n >= NUM_COLS) {
            throw new IllegalArgumentException("Invalid index: Out of range"); //need this if the dimesions dont match the matrix
        }

        double rowAverage = sumRow(matrix, n) / NUM_COLS; //sum at n for row and collumn and get average
        double colAverage = sumCol(matrix, n) / NUM_ROWS;

        return Math.max(rowAverage, colAverage); //return max
    }
}
